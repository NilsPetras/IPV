...
[*Dantlgraber, M., Stieger, S., & Reips, U.-D., 2018, Introducing Item Pool Visualization (IPV). GSDS Working Paper*](https://www.gsds.uni-konstanz.de/typo3temp/secure_downloads/64291/0/c11fa59f405bed86279fa8eb67d5d80b72685027/WP2018-08_DantlgraberStiegerReips.pdf)  
...
mychart <- item_chart(data = DSSEI, file_name = "christmas")
mychart
```

As you can see, getting a first result is meant to be trivial. All three IPV chart types can be created by specifying `data = ` only. (Here, `file_name = ` is specified with no file extension to suppress the file output.)  
Although I have put considerable effort in setting sensible defaults (or generating them dynamically, based on the data), it is always possible to optimize the charts. For example, I prefer my charts to have a higher [data ink ratio](https://infovis-wiki.net/wiki/Data-Ink_Ratio) and like to use [color](http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf) to guide attention. The many graphical parameters enable e.g. to create a black and white print optimized version or a version optimized for small display.

## Workflow
Assuming you have some interesting raw data, empirical, simulated, or manufactured mischievously, these are the necessary steps to produce IPV charts:  
0. SEM estimation  
1. Input  
2. Chart creation  
...
### 0. SEM estimation
The IPV package does **not** do the SEM estimation for you. But that means you get to use your favorite statistical software (e.g. the R packages [lavaan](http://lavaan.ugent.be/) or [sem](https://socialsciences.mcmaster.ca/jfox/Misc/sem/SEM-paper.pdf)). The [abovementionned paper](https://www.gsds.uni-konstanz.de/typo3temp/secure_downloads/64291/0/c11fa59f405bed86279fa8eb67d5d80b72685027/WP2018-08_DantlgraberStiegerReips.pdf) shows how to specify the SEM models. For the example used here and in the paper, you might want to take a look at figures 6-8, showing the three SEMs used. As the authors point out, the models can only be estimated as SEMs, not by means of factor analysis or in any other way.   
...
```
Note, that `factor` refers to an item pool, that was divided into subpools (`subfactor`s). In this case, `factor` refers to the Rosenberg Self-Esteem Scale, a test with two facets: Positive Self-Esteem (Ps), and Negative Self-Esteem (Ns), which was reversed here. As seen below, the same data structure applies on the global level, with `factor` refering to the overall self-confidence item pool, comprising the three tests (`subfactor`s) RSES, SMTQ, and DSSEI.  
```{r}
...
#### Using Excel files
Excel files have the advantage, that you can just copy and paste your SEM estimates into the spreadsheets and the input function of the IPV package (`input_excel()`) does the rest. The files need to be structured as in the example, that you can find here:
```{r}
...
Generally, best results can be achieved using .pdf files, since they are vector based. .pdf files can be zoomed and scaled indefinitely without loss of quality. Pixelated formats (.png and .jpeg are supported) will lead to lower quality results and are not scalable. The parameters `file_width = ` and `file_length = ` determine, how large the .pdf file will be, measured in inches (1 in = 2.54 cm). The size of .png or .jpeg files in pixels is determined by multiplying the size in inches with the dots per inch parameter value (`dpi = `). To inspect your results within RStudio, always use the zoom popout of the Plots window, otherwise charts are heavily distorted.  
...
For all chart types, it is possible to rotate the whole chart, using `rotate_radians = ` or `rotate_degrees = `. Also, the font can be changed, using the `font = ` parameter. I recommend using the package [extrafont](https://cran.r-project.org/web/packages/extrafont/README.html) for access to more fonts.
...
In the following, I will go into detail on the individual chart types. Using the example, I will show the impotant customization options provided in the chart functions:
...
```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- item_chart(data = DSSEI, file_name = "cucumber",
...
```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- item_chart(data = DSSEI, file_name = "handcuffs",
                      color = "darkblue", color2 = "darkblue",
                      fade_axes = 70, fade_grid_major = 50, fade_grid_minor = 92,
...
```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- item_chart(data = DSSEI, file_name = "gnome",
...
```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px", fig.show='hold'}
x <- DSSEI
...
mychart1 <- item_chart(data = x, file_name = "squirrel")
mychart2 <- item_chart(data = x, file_name = "squirrel", dodge = 7)
mychart1
mychart2
```
...
```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- facet_chart(data = DSSEI, file_name = "hachoo")
mychart
```
...
```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- facet_chart(data = DSSEI, file_name = "ants",
                      subradius = .1)
mychart
```
In this example, the radius of the facet circles is too small, to display the facet names and correlations in a reasonable size. In the example below, it is too large, so center distances do not shape the overall impression anymore, and the facet circles even overlap.
```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- facet_chart(data = DSSEI, file_name = "giants",
                      subradius = 1.5)
mychart
```
...
```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- facet_chart(data = DSSEI, file_name = "organ",
                      cor_labels = FALSE)
mychart
```
...
```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- facet_chart(data = DSSEI, file_name = "south",
                      rotate_test_label_radians = pi, color = "firebrick4")
mychart
```

#### Nested charts
Nested charts are the most complex IPV charts. In addition to the abovementionned topics, there are four important considerations: the `relative_scaling = ` of the global and the nested level, the addition of `xarrows = ` to display correlation arrows between facets of different tests, the ability to `subrotate_... = ` each test individually, and the `cor_spacing = ` to display correlations between the tests. Due to the complexity one should not rely on default values too much:

```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- nested_chart(data = self_confidence, file_name = "labyrinth")
mychart
```
...
```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- nested_chart(data = self_confidence, file_name = "aha",
...
mychart <- nested_chart(data = self_confidence, file_name = "needle",
                        subradius = .5, size_facet_labels = 2, size_cor_labels_inner = 1.5,
...
Now the arrows create a lot of overlap and make the chart look messy. This problem can be solved by rotating each of the nested facet charts, so the facets connected by arrows are oriented towards the center. Also the construct label should be moved out of harms way.

```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- nested_chart(data = self_confidence, file_name = "wobble",
                        subradius = .5, size_facet_labels = 2, size_cor_labels_inner = 1.5,
                        xarrows = sc_arrows, show_xarrows = TRUE,
                        subrotate_degrees = c(180, 270, 90), rotate_construct_label_degrees = -20)
mychart
```
...
```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- nested_chart(data = self_confidence, file_name = "bad",
                        subradius = .5, size_facet_labels = 2, size_cor_labels_inner = 1.5,
                        xarrows = sc_arrows, show_xarrows = TRUE,
                        subrotate_degrees = c(180, 270, 90), rotate_construct_label_degrees = -20,
...
mychart
```
To get a somewhat decent result, let us change some `size_... = ` parameters and add some color. Color can be chosen for the global and the nested level independently. Furthermore, it might be better to increase the line thickness, so the impression of connected colored elements clarifies.

```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
mychart <- nested_chart(data = self_confidence, file_name = "tadaa",
                        subradius = .5, size_facet_labels = 2, size_cor_labels_inner = 1.5,
                        xarrows = sc_arrows, show_xarrows = TRUE,
                        subrotate_degrees = c(180, 270, 90), rotate_construct_label_degrees = -20,
                        color_global = "darkorchid", color_nested = "darkblue",
...
mydata <- input_manual_nested(
  ...
  correlation_matrix = matrix(data = c(  1, .73, .62,
                              ...
                              nrow = 3,
                              ncol = 3))
# tests have to be added individually
