...
<span style="color:darkblue">Blue bars</span> represent items. Center distances are displayed on radial axes (arrows). They are the distance from the origin to the inner edge of the bars (not the center of the bars!). The items are arranged by facets on the radial axes. Read center distance values from the grid of concentric circles, where steps of 0.5 are highlighted.
...
SMTQ_facets <- facet_chart(SMTQ, file_name = "none")
SMTQ_facets
```
...
- **size** scales (almost) all elements of the chart. This is the most important cosmetic parameter.
