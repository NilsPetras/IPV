...
1a. estimate models (recommended)
...
- the data frame contains only those variables used in the models
...
The font can be changed using the `font = ` parameter. I recommend using the package [extrafont](https://cran.r-project.org/package=extrafont) for access to more fonts.
...
mychart
```
As you can see in the output (and the message in the console), two parameter values (`subradius = ` , and `tick = ` ) have been generated automatically. These need to fit the data. 
