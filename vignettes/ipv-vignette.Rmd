...
```
Example for a nessted IPV plot. All example data in this vignette from: *Dantlgraber, M., Stieger, S., & Reips, U.-D. (in revision). Introducing Item Pool Visualization (IPV)*
...
Functions are named according to the plot concept (e.g. model_items and model_facets). In any case you will have to use two functions in conjunction. The first function is the *model function* and prepares coordinates and labels for plot objects. The second function is the *plot function* and creates both a ggplot object and a .pdf file, the actual plot. So for each plot concept there are two functions.

For example, creating an item plot works like this:

```{r, fig.width=10, fig.height=10, dpi=300, out.height="685px", out.width="685px"}
coord <- model_items(DSSEI)
DSSEI_item_plot <- plot_items(coord,filename = "DSSEI_items",font = "sans")
DSSEI_item_plot
```

As you see, almost all function parameters have a default value, so you can start fast and frugal.

There is a useful shortcut if you have a nested model. The function model_nested always also generates all you need for facet plots of all correlated factors. If you set items = TRUE, you also get all you need for the item plots in one go:

```{r,eval=FALSE}
coord <- model_nested(self_confidence,subradius = .6,items = TRUE)
...
- *extra_arrows* (_nested) adds custom correlation arrows. Provide information about latent correlations between facets of different factors to the model function (as below). Set *extra_arrows*=TRUE in plot_nested. Straighten out your plot with *subrotate* (model_nested).
