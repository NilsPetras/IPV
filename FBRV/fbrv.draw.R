fbrv.draw <- function(coor,size=1,file_name,width=10,height=10,ticklabel=FALSE,
                      cor_labels=TRUE,subfactor_cor_labels=FALSE,
                      colour="black",subcolour="black",extra_arrows=FALSE,tick=.1,
                      size_title=1,size_subfactor_labels=1,size_subfactor_labels_inner=1,width_axes=1,width_axes_inner=1,
                      width_circles=1,width_circles_inner=1,size_tick=1,size_tick_inner=1,size_cor_labels=1,size_cor_labels_inner=1,
  ...
  globalplot <- ggplot(coor$global$cart_circles)+
    ...
    geom_segment(data = coor$global$nested$axes,aes(x=x2,y=y2,xend=x3,yend=y3),size=.25*(sqrt(size)+size)*width_axes_inner,color="gray90")+
    ...
    geom_text(data=coor$global$nested$circles,aes(x,y,label = label),family = "serif",size = 2.5*sqrt(size)*size_subfactor_labels_inner)+
    ...
    geom_text(data = coor$global$factor_label,aes(x=x,y=y,label=label),family = "serif",size = 6*sqrt(size)*size_title,fontface="bold")+
    geom_text(data = coor$global$nested$factor_label,aes(x=x,y=y,label=label),family = "serif",size = 4*sqrt(size)*size_subfactor_labels,fontface="bold")+
  ...
  if(!is.null(subfactor_inner_cors)){globalplot <- globalplot +
    geom_text(data = subfactor_inner_cors,aes(x=x,y=y,label=label),family = "serif",size = 1.5*sqrt(size)*size_cor_labels_inner)}
  if(!is.null(inner_cors)){globalplot$layers <- c(geom_circle(data=coor$global$cart_inner_ring,aes(x0=x,y0=y,r=radius),size=.5*size*width_axes_inner,color="gray90"),globalplot$layers)
    globalplot <-  globalplot + geom_text(data = inner_cors,aes(x=x,y=y,label=label),family = "serif",size = coor$global$cor_spacing*5*size*size_cor_labels,fontface="bold")}
  if(extra_arrows==TRUE){globalplot <- globalplot +
    ...
    geom_text(data = coor$global$arrows,aes(x=xlabel,y=ylabel,label=label),size=2*sqrt(size)*size_extra_labels,family="serif",color="gray20")}
