fbrv.plot.items <- function(coor,size=1,file_name,
  ...
  myfbrv <- ggplot(coor$cart_axes)+
    geom_text(aes(x=xlabel,y=ylabel,label = row.names(coor$cart_axes)),family = "serif",size = 6*sqrt(size)*size_axes_labels,hjust = "inward")+
    coord_fixed()+
    theme_minimal()+
    aes()+
    geom_point(aes(x=0,y=0),size=.5*size*size_center_dot)+
    geom_circle(data = coor$maingrid[-seq(from = 5, by = 5, to = length(coor$maingrid$alpha)),],aes(x0=x,y0=y,r=r),col = "gray90",linetype = "dotted",size=min(c(size,.75))*size_grid)+
    geom_circle(data = coor$maingrid[seq(from = 5, by = 5, to = length(coor$maingrid$alpha)),],aes(x0=x,y0=y,r=r),col = "gray20",linetype = "dotted",size=min(c(size,.75))*size_grid)+
    ...
    geom_text(data = coor$factor_label,aes(x=x,y=y,label = label),family = "serif",size = 6*sqrt(size)*size_title,color=colour,fontface="bold")+
  ...
  if(tick_label == TRUE && max(coor$maingrid$r)>=.5) myfbrv <- myfbrv +  geom_text(data = coor$axis_tick,aes(x,y,label = "0.5"),angle = (coor$axis_tick$phi-pi/48-pi/2)*180/pi,family = "serif",size = 2*size*size_tick_label,col = "gray20")
