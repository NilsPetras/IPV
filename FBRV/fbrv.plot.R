fbrv.plot <- function(coor,size=1,file_name,
                      width=10,height=10,colour="black",cor_labels=TRUE,tick=.1,
                      size_title=1,size_subfactor_labels=1,width_axes=1,width_circles=1,size_tick=1,size_cor_labels=1,size_center_dot=1){
  ...
  myfbrv <- ggplot(coor$cart_circles)+
    ...
    geom_text(aes(x,y,label = row.names(coor$cart_circles)),family = "serif",size = 8*sqrt(size)*size_subfactor_labels)+
    coord_fixed()+
    theme_minimal()+
    aes()+
    ...
    geom_text(data = coor$factor_label,aes(x=x,y=y,label=label),family = "serif",size = 8*sqrt(size)*size_title,fontface="bold")+
  ...
  if(!is.null(inner_cors))myfbrv <- myfbrv +
    geom_text(data = inner_cors,aes(x=x,y=y,label=label),family = "serif",size = 4*sqrt(size)*size_cor_labels)
