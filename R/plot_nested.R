...
#' @param cor_labels_tests logical; if \code{TRUE}, draws latent correlations between
#'   tests as text.
...
#'   default).
#' @param size_test_labels integer; font size of the test labels (relative
#'   to default).
...
plot_nested <- function(coor, size = 1, filesave=TRUE, filename=NULL, filewidth = 10, fileheight = 10,
  ...
  globalplot <- ggplot2::ggplot(coor$global$cart_circles)+
    ...
    ggplot2::geom_segment(data = coor$global$cart_axes,ggplot2::aes(x=x0,y=y0,xend=x1,yend=y1),size=.5*(sqrt(size)+size)*width_axes,color=colour_tests)+
    ggforce::geom_circle(data=coor$global$nested$circles,ggplot2::aes(x0=x,y0=y,r=radius),size=.25*size*width_circles_inner,color=colour_facets)+
    ggplot2::geom_text(data=coor$global$nested$circles,ggplot2::aes(x,y,label = label),family = font,size = 3.125*sqrt(size)*size_facet_labels)+
    ggplot2::geom_segment(data = coor$global$nested$axes,ggplot2::aes(x=x0,y=y0,xend=x1,yend=y1),size=.25*(sqrt(size)+size)*width_axes_inner,color=colour_facets)+
    ...
    ggplot2::geom_text(data = coor$global$nested$factor_label,ggplot2::aes(x=x,y=y,label=label),family = font,size = 4*sqrt(size)*size_test_labels,fontface="bold")+
  ...
  # saving as .pdf file
  if(filesave == TRUE) ggplot2::ggsave(paste(filename,".pdf",sep = ""),globalplot,width = filewidth ,height = fileheight, units = "in",dpi = 3000)
